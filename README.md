...
- 🔗 **URL参数提取**: 自动从URL参数中提取真实的m3u8地址（如`?url=https://...m3u8`）
- 🔄 **递归解析**: 自动递归解析所有嵌套的m3u8地址，确保找到真实的视频流地址
...
- **智能URL参数提取**: 如果解析出的URL包含参数（如`https://example.com/player.php?url=https://real-video.m3u8`），系统会自动从参数中提取真实的视频地址
- **递归解析嵌套地址**: 系统会自动递归解析所有m3u8地址，包括：
  - 从URL参数中提取的地址
  - m3u8文件内容中的嵌套地址（如主播放列表中的子播放列表）
  - 相对路径的m3u8地址（如`2000k/hls/mixed.m3u8`）
  - 所有找到的地址都会列出供用户选择
...
系统支持从包含参数的URL中自动提取真实的视频地址。这对于某些视频网站特别有用，这些网站将真实的视频URL作为参数传递给播放器页面。

**示例：**
- 原始URL: `https://example.com/player.php?url=https://real-video.com/video.m3u8`
- 提取结果: `https://real-video.com/video.m3u8`

**支持的参数名：**
- `url`, `m3u8`, `src`, `source`, `video`, `stream`, `play`, `file`, `link`, `path`, `v`, `u`, `s`

**提取时机：**
- 在解析阶段自动提取，确保返回给用户的是真实的视频地址
- 如果下载时发现URL返回HTML内容，也会尝试从HTML中提取真实的m3u8地址
...
系统支持递归解析所有嵌套的m3u8地址，确保找到真实的视频流地址。这对于主播放列表（Master Playlist）特别有用。

**解析逻辑：**
1. **参数检测**: 首先检查URL是否是参数形式，如果是则提取真实地址并递归解析
2. **内容获取**: 使用requests直接获取m3u8文件内容（更可靠），失败时回退到Playwright
3. **内容解析**: 解析m3u8文件内容，查找所有嵌套的m3u8地址：
   - `#EXT-X-STREAM-INF` 标记后的URL（主播放列表）
   - 所有包含`.m3u8`的非注释行
   - URL参数格式（如`url=https://...m3u8`）
   - 完整的http(s)://...m3u8 URL
4. **相对路径处理**: 自动将相对路径转换为绝对路径：
   - 绝对路径: `/path/to/video.m3u8` → `https://domain.com/path/to/video.m3u8`
   - 相对路径: `2000k/hls/mixed.m3u8` → `https://domain.com/dir/2000k/hls/mixed.m3u8`
5. **递归处理**: 对所有找到的嵌套地址递归解析，直到找到真实的视频流地址
6. **全部列出**: 所有找到的地址都添加到列表，供用户选择

**示例：**
```
解析: https://example.com/index.m3u8
    ↓
获取内容，发现包含: 2000k/hls/mixed.m3u8
    ↓
构建完整URL: https://example.com/2000k/hls/mixed.m3u8
    ↓
递归解析这个URL
    ↓
所有地址都列出供用户选择
...
**深度解析：**
- 如果标准解析没有找到嵌套地址，系统会使用更宽泛的正则模式进行深度解析
- 对于小文件（可能是主播放列表），会打印完整内容以便调试
- 支持JavaScript执行，获取动态加载的m3u8地址

**安全机制：**
- 最大递归深度：3层（防止无限循环）
- 循环检测：防止重复访问同一URL
- 超时控制：HTTP请求超时15秒
- 错误处理：异常时仍将URL添加到列表，不中断流程

### M3U8下载增强

系统增强了M3U8下载的健壮性：
- **内容验证**: 验证M3U8文件格式，确保不是HTML内容
- **URL过滤**: 过滤掉HTML标签和脚本内容，只解析有效的TS片段URL
- **响应验证**: 验证HTTP响应的Content-Type和内容，确保下载的是视频数据
- **文件验证**: 验证下载的文件内容，防止将HTML保存为视频文件
...
### v3.2.0
- ✨ 新增M3U8递归解析功能：自动递归解析所有嵌套的m3u8地址
  - 支持解析主播放列表（Master Playlist）中的子播放列表
  - 支持相对路径自动转换（如`2000k/hls/mixed.m3u8`）
  - 优先使用requests获取m3u8内容（更可靠），失败时回退到Playwright
  - 使用多种模式查找嵌套地址：`#EXT-X-STREAM-INF`、包含`.m3u8`的行、URL参数格式等
  - 深度解析：即使标准解析未找到，也会使用更宽泛的模式进行深度解析
  - 支持JavaScript执行，获取动态加载的m3u8地址
  - 所有找到的地址都会列出供用户选择
- 🔧 改进URL参数提取：保留完整URL（包括查询参数），这些参数可能是必需的
- 🔧 改进相对路径处理：正确处理各种相对路径格式
- 📝 更新文档，添加递归解析功能说明

### v3.1.0
- ✨ 新增URL参数提取功能：自动从URL参数中提取真实的m3u8地址
  - 支持从查询参数（如`?url=https://...m3u8`）中提取真实地址
  - 支持从URL fragment（`#...m3u8`）中提取真实地址
  - 支持多重URL解码（处理多重编码的情况）
  - 支持常见参数名：`url`, `m3u8`, `src`, `source`, `video`, `stream`, `play`, `file`, `link`, `path`, `v`, `u`, `s`
  - 在解析阶段自动提取，无需等到下载时才处理
- 🐛 修复M3U8下载失败问题：增强M3U8内容验证，防止下载HTML内容作为视频片段
- 🐛 修复TS片段URL解析问题：过滤HTML标签和脚本内容，确保只解析有效的视频URL
- 📝 更新文档，添加URL参数提取功能说明
